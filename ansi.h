...
#define __ansi_H
    ...
    #define CURSOR_DOWN(n) printf(ANSI_START "%dB", (int)n)                  // Move cursor down n lines
    #define CURSOR_FORWARD(n) printf(ANSI_START "%dC", (int)n)               // Move cursor forward n columns
    #define CURSOR_BACK(n) printf(ANSI_START "%dD", (int)n)                  // Move cursor back n columns
    #define CURSOR_NEXT_LINE(n) printf(ANSI_START "%dE", (int)n)             // Move cursor to the beginning of the line n lines down
    #define CURSOR_PREVIOUS_LINE(n) printf(ANSI_START "%dF", (int)n)         // Move cursor to the beginning of the line n lines up
    #define CURSOR_HORIZONTAL_ABSOLUTE(n) printf(ANSI_START "%dG", (int)n)   // Move cursor to column n
    #define CURSOR_TO(n, m) printf(ANSI_START "%d;%dH", (int)n, (int)m)           // Move cursor to row n, column m
    #define CURSOR_TO_ALT(n, m) printf(ANSI_START "%d;%df", (int)n, (int)m)       // Alternative command to move cursor to row n, column m
    // Erase and scroll commands
    #define ERASE_DISPLAY(n) printf(ANSI_START "%dJ", (int)n)           // Erase part of the display
    ...
    #define MODE_TO_END 0 // Erase from the current cursor position to the end of the current line
    #define MODE_TO_BEGINNING 1 // Erase from the current cursor position to the start of the current line
    #define MODE_ALL 2 // Erase the entire current line

    // Non-standard ANSI commands for cursor saving and restoring
    #define CURSOR_SAVE() printf(ANSI_START "s")    // Save cursor position (non-standard)
    #define CURSOR_RESTORE() printf(ANSI_START "u") // Restore cursor position (non-standard)
    ...
    #define __ANSI_COLORS_256_H
            ...
            #define COLOR_256_FG(n) (ANSI_START "38;5;" + std::to_string(n) + "m").c_str() // 256 foreground color
            #define COLOR_256_BG(n) (ANSI_START "48;5;" + std::to_string(n) + "m").c_str() // 256 background color
        #else
            // Extended ANSI color definitions
    ...
    #define ANSI_COLORS_24bit_H
            ...
            #define COLOR_24bit_FG(r, g, b) (ANSI_START "38;2;" + std::to_string(r) + ";" + std::to_string(g) + ";" std::to_string(b) "m").c_str() // 24-bit foreground color
            #define COLOR_24bit_BG(r, g, b) (ANSI_START "48;2;" + std::to_string(r) + ";" + std::to_string(g) + ";" std::to_string(b) "m").c_str() // 24-bit background color
        #else
            //24-bit ANSI color definitions
            #define COLOR_24bit_FG(r, g, b) ANSI_START "38;2;" #r ";" #g ";" #b "m" // 24-bit foreground color
            #define COLOR_24bit_BG(r, g, b) ANSI_START "48;2;" #r ";" #g ";" #b "m" // 24-bit background color
        #endif
    #endif
#endif
